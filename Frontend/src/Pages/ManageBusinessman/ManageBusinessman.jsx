...
const mockBusinessmen = [
  {
    ...
    email: "robert.anderson@business.com",
    username: "robanderson",
    phone: "+1234567890",
    city: "New York",
    type: "service",
    status: "active",
  ...
  },
  {
    id: 2,
    name: "Sarah Williams",
    email: "sarah.williams@business.com",
    username: "sarahw",
    phone: "+1234567891",
    city: "Los Angeles",
    type: "product",
    ...
    dob: "1988-07-12",
    joinDate: "2023-02-20",
    businessName: "Williams Electronics",
  },
  {
    id: 3,
    name: "Michael Chen",
  ...
  ...Array.from({ length: 47 }, (_, i) => ({
    id: i + 4,
    name: `Businessman ${i + 4}`,
    email: `businessman${i + 4}@business.com`,
    username: `business${i + 4}`,
    phone: `+123456789${i + 4}`,
    city: ["New York", "Los Angeles", "Chicago", "Houston", "Phoenix"][i % 5],
    type: i % 2 === 0 ? "service" : "product",
    status: ["active", "suspended", "blocked"][i % 3],
    isEmailVerified: Math.random() > 0.5,
    isPhoneVerified: Math.random() > 0.5,
    dob: `198${2 + (i % 5)}-0${1 + (i % 9)}-${10 + (i % 20)}`,
    joinDate: "2023-04-01",
    businessName: `Business ${i + 4} Inc.`,
  })),
];
...
const SearchInput = ({
  ...
  <div className="relative flex-1 max-w-2xl">
    ...
    <input
      type="text"
      placeholder="Search businessmen by name, email, or username..."
      value={searchTerm}
...
const StatusActionDropdown = ({ businessman, onStatusChange, isMobile = false }) => {
  ...
  const availableOptions = statusOptions.filter(
    (option) => option.value !== businessman.status
  );
  ...
  const getCurrentStatusConfig = () => {
    return statusOptions.find((option) => option.value === businessman.status);
  };
                ...
                {availableOptions.map((option) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() => {
                        onStatusChange(businessman.id, option.value);
                        setIsOpen(false);
                      }}
...
// BusinessmanDetailsModal Component
const BusinessmanDetailsModal = ({ businessman, isOpen, onClose }) => {
  useEffect(() => {
    if (isOpen) {
  ...
  if (!isOpen || !businessman) return null;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div className="p-3 sm:p-4 md:p-5 bg-gray-50 rounded-xl shadow-sm">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <p className="text-base sm:text-lg md:text-xl font-semibold text-gray-800 flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <span className="truncate">{businessman.email}</span>
                  </p>
                  <span
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 sm:p-4 md:p-5 bg-gray-50 rounded-xl shadow-sm">
                ...
                <div className="mt-2">
                  <TypeBadge type={businessman.type} />
                </div>
              </div>
              ...
              <div className="p-3 sm:p-4 md:p-5 bg-gray-50 rounded-xl shadow-sm">
                ...
                <p className="text-base sm:text-lg md:text-xl font-semibold text-gray-800 flex items-center gap-2">
                  ...
                  {new Date(businessman.dob).toLocaleDateString()}
                </p>
              </div>
              ...
              <div className="p-3 sm:p-4 md:p-5 bg-gray-50 rounded-xl shadow-sm">
                ...
                <div className="mt-2">
                  <StatusBadge status={businessman.status} />
                </div>
              </div>
            </div>
          </div>
...
export default function AdminBusinessmanManagement() {
  ...
  const filteredBusinessmen = mockBusinessmen
    .filter(
      (businessman) =>
        ...
        businessman.username.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .filter((businessman) => {
      if (sortBy === "blocked") return businessman.status === "blocked";
      if (sortBy === "service") return businessman.type === "service";
      if (sortBy === "product") return businessman.type === "product";
      return true;
    })
    .sort((a, b) => {
  ...
  const totalPages = Math.ceil(filteredBusinessmen.length / businessmenPerPage);
  const startIndex = (currentPage - 1) * businessmenPerPage;
  const currentBusinessmen = filteredBusinessmen.slice(
    startIndex,
  ...
  const handleViewDetails = (businessman) => {
    setSelectedBusinessman(businessman);
    setShowModal(true);
  };

  const handleBusinessmanAction = (businessmanId, action) => {
        ...
        >
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <p className="text-lg sm:text-2xl font-bold text-gray-800">
                  {mockBusinessmen.filter((b) => b.type === "service").length}
                </p>
                <p className="text-gray-600 text-xs sm:text-sm font-medium">
                  Service
                </p>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <p className="text-lg sm:text-2xl font-bold text-gray-800">
                  {mockBusinessmen.filter((b) => b.type === "product").length}
                </p>
                <p className="text-gray-600 text-xs sm:text-sm font-medium">
                  Product
                </p>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <p className="text-lg sm:text-2xl font-bold text-gray-800">
                  {mockBusinessmen.filter((b) => b.status === "blocked").length}
                </p>
                <p className="text-gray-600 text-xs sm:text-sm font-medium">
                  Blocked
                </p>
        ...
        >
          <div className="hidden md:block overflow-x-auto overflow-hidden">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {currentBusinessmen.map((businessman, index) => (
                  <motion.tr
                    key={businessman.id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center shadow-lg">
                          <span className="text-white font-bold text-sm">
                            {businessman.name.charAt(0).toUpperCase()}
                          </span>
                        </div>
                        <div>
                    ...
                    <td className="px-6 py-5 relative">
                      <div className="flex items-center justify-center gap-3 relative">
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => handleViewDetails(businessman)}
                        ...
                        <motion.button
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          onClick={() => handleEmailClick(businessman.email)}
      ...
      <AnimatePresence>
        {showModal && (
          <BusinessmanDetailsModal
            businessman={selectedBusinessman}
            isOpen={showModal}
