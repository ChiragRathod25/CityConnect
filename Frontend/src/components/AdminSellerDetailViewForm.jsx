...
// --- 1. REAL API DATA SIMULATION ---
// In a real app, this comes from your fetch/axios response
const rawApiResponse = [
  {
    "_id": "6906498db7f1561d678bcb7c",
    "ownerId": "68f47901d8d1e7275a2c99c8",
    "name": "Spice Garden",
    "type": "product",
    "category": "restaurant",
    "description": "Authentic Indian dining experience offering traditional curries, tandoori dishes, and regional specialties.",
    "logo": null,
    "images": [],
    "status": "pending",
    "isVerified": false,
    "rejectedReason": null,
    "createdAt": "2025-11-01T17:55:25.658Z",
    "updatedAt": "2025-11-01T17:55:25.658Z",
    "__v": 0,
    "ownerDetails": [
      {
        "_id": "68f47901d8d1e7275a2c99c8",
        "username": "one1",
        "email": "cityconnect.owner@gmail.com",
        "phoneNumber": "8160279524",
        "image": null,
        "status": "active",
        "role": "admin",
        "isEmailVerified": true,
        "isPhoneVerified": true,
        "avatar": "https://res.cloudinary.com/dc4undf3d/image/upload/v1760905406/zuawz8aryh0vvd6he8pw.jpg",
        "firstName": "onetwothree",
        "lastName": "two"
      }
    ],
    "locationDetails": [
      {
        "_id": "69064997b7f1561d678bcb8b",
        "businessId": "6906498db7f1561d678bcb7c",
        "street": "221 Curry St",
        "lat": 37.7749,
        "lng": -122.4194,
...
// --- 2. HELPER: TRANSFORM API DATA TO UI STRUCTURE ---
const transformBusinessData = (item) => {
  const owner = item.ownerDetails?.[0] || {};
  const location = item.locationDetails?.[0] || {};
  const contact = item.contactDetails?.[0] || {};
  const workingHours = item.workingHoursDetails || [];

  // Determine Operating Hours (e.g., take Monday or first open day)
  const todaySchedule = workingHours.find(day => !day.isClosed) || {};
  
  // Find closed days
  const closedDays = workingHours.filter(day => day.isClosed).map(d => d.dayOfWeek);
  
  return {
    id: item._id,
    submittedAt: item.createdAt,
    status: item.status, // "pending", "approved", "rejected"
    rejectReason: item.rejectedReason,
    personalInfo: {
    ...
    businessInfo: {
      ...
      yearsInBusiness: "N/A", // Not provided in API
    },
    location: {
      address: location.street || location.address,
      city: location.city,
      district: location.state, // Fallback if district not in API
      state: location.state,
      pincode: location.postalCode,
    },
    businessSettings: {
      // These boolean flags are not in current API, defaulting to false or checking existence
      deliveryAvailable: false, 
      homeServiceAvailable: false,
      paymentMethods: ["Cash", "UPI"], // Default placeholder
      openingTime: todaySchedule.openTime || "N/A",
      closingTime: todaySchedule.closeTime || "N/A",
      weeklyOff: closedDays.length > 0 ? closedDays.join(", ") : "None",
    },
    documents: {
      // Mapping API images to UI slots
      govtIdProof: null, // Not in API
      selfieWithId: null, // Not in API
      businessLicense: null, // Not in API
      businessPhoto: item.images?.[0] || null,
      professionalPhoto: owner.avatar,
    },
  };
};

// --- 3. UI COMPONENTS (Unchanged utility components) ---
        ...
        <div className="p-4 sm:p-6 bg-gradient-to-br from-gray-50 via-white to-gray-50 border-b border-gray-100">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-black to-gray-700 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg overflow-hidden">
                {/* Fallback to first image or generic icon if no logo/photo */}
                {application.documents.businessPhoto ? (
                    <img src={application.documents.businessPhoto} alt="Business" className="w-full h-full object-cover" />
                ) : (
...
// --- 5. MAIN PANEL: FETCHING AND STATE MANAGEMENT ---
const AdminVerificationPanel = () => {
  ...
  const fetchAllBusinesses=async ()=>{
    const response=await databaseService.getAllBusinessesWithAllDetails();
    return response.data;
  }
  useEffect(() => {
    fetchAllBusinesses().then((data) => {
      const formattedData = data.map(transformBusinessData);
      setApplications(formattedData);
    });
  }, []);

  const handleApprove = async (appId) => {
    console.log(`Making API call to approve business: ${appId}`);
    
    const response=await databaseService.updateBusinessStatus(appId,"active");
    
    
    setApplications((prev) =>
      prev.map((app) =>
        app.id === appId ? { ...app, status: "active" } : app
      )
    );
    setTimeout(
      () => alert(`✓ Application ${appId.slice(-6)} has been approved successfully!`),
      100
    );
  };

  const handleReject = async (appId, reason) => {
    console.log(`Making API call to reject business: ${appId} with reason: ${reason}`);
    // Example: axios.put(`/api/admin/verify/${appId}`, { status: 'rejected', reason })
    const response=await databaseService.updateBusinessStatus(appId,"suspended");

    setApplications((prev) =>
      prev.map((app) =>
        app.id === appId
          ? { ...app, status: "suspended", rejectReason: reason }
          : app
      )
    );
    setTimeout(() => alert(`✗ Application ${appId.slice(-6)} has been rejected.`), 100);
  };
  ...
  const stats = {
    ...
    approved: applications.filter((app) => app.status === "active").length,
    rejected: applications.filter((app) => app.status === "suspended").length,
  };
