...
const FormSelect = ({ label, icon: Icon, options = [], error, className = "", ...props }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedOption, setSelectedOption] = useState(options.find(opt => opt.value === props.value) || null);
...
const MapComponent = ({ onLocationSelect, selectedLocation, error }) => {
  ...
  const loadGoogleMapsScript = () => {
    ...
    script.onerror = () => {
      setMapError('Failed to load Google Maps. Using alternative map interface.');
      setIsMapLoaded(true);
    ...
    try {
      ...
      if (window.google) {
        ...
        addressData = {
          lat: selectedCoords.lat,
          lng: selectedCoords.lng,
          address: `Near ${Math.floor(Math.random() * 100)} Main Street, Block ${Math.floor(Math.random() * 20)}`,
          city: "Vadodara",
          district: "Vadodara",
          state: "Gujarat",
  ...
  const handleManualLocationSubmit = () => {
    if (manualLocation.address && manualLocation.city && manualLocation.state && manualLocation.pincode) {
      onLocationSelect({
        lat: 0,
        lng: 0,
        ...manualLocation
            ...
            >
              ...
              >
                <div className="p-4 border-b border-gray-200 flex items-center justify-between">
                  <h3 className="text-lg font-semibold">Select Your Business Location</h3>
                  <button
                    onClick={() => {
                      setShowMap(false);
                      setSelectedCoords(null);
                ...
                <div className="relative">
                    ...
                    >
                      ...
                      {selectedCoords && !window.google && (
                        <motion.div
                          ...
                          style={{
                            left: `${50 + (selectedCoords.lng - 73.1812) * 1000}%`,
                            top: `${50 - (selectedCoords.lat - 22.3039) * 1000}%`
                          }}
                        >
                  ...
                  {!selectedCoords && (
                    <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-80 text-white px-6 py-3 rounded-lg text-sm">
                      🎯 Click anywhere on the map to select your business location
                    </div>
                  )}
                </div>
            ...
            >
              ...
              <div className="flex gap-3 pt-2">
                ...
                <motion.button
                  ...
                  className="flex items-center gap-2 bg-gray-500 text-white px-6 py-2 rounded-lg font-medium hover:bg-gray-600 transition-colors"
                >
                  <X size={16} />
                  Cancel
                </motion.button>
          ...
          >
            ...
            {selectedLocation.city && (
              <p className="text-sm text-green-600 mt-1">
                📍 {selectedLocation.city}, {selectedLocation.district}, {selectedLocation.state} - {selectedLocation.pincode}
              </p>
            )}
...
const FileUpload = ({ label, accept, onChange, icon: Icon, description, preview, error }) => (
  <div className="space-y-2">
    <label className="flex items-center gap-2 text-sm font-semibold text-gray-700">
      {Icon && <Icon size={16} className="text-gray-500" />}
      {label}
    ...
    <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-all duration-300 ${
      error ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50'
    }`}>
      <input
        type="file"
        accept={accept}
      ...
      >
        {preview ? (
          <div className="space-y-3">
            <img src={preview} alt="Preview" className="w-20 h-20 mx-auto rounded-lg object-cover shadow-md" />
...
const ModernSellerForm = () => {
  ...
  const handleFileUpload = (field, event) => {
    ...
    if (file) {
      ...
      if (allowedTypes[field] && !allowedTypes[field].includes(file.type)) {
        ...
        const allowed = allowedTypes[field].map(type => fileTypeNames[type]).join(', ');
        setErrors(prev => ({ ...prev, [field]: `Only ${allowed} files are allowed` }));
        return;
      }
    ...
    switch (step) {
      ...
      case 4:
        if (!formData.businessLicense) {
          newErrors.businessLicense = 'Business license/registration document is required';
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert('Registration successful! Welcome to CityConnect. You will receive a confirmation email shortly.');
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="relative">
                ...
                >
                  {showPassword ? <div className='py-2'><EyeOff size={20} /></div> : <div className='py-2'><Eye size={20} /></div>}
                </motion.button>
              </div>
            </div>
          </motion.div>
            ...
            <div>
              ...
              <textarea
                ...
                className={`w-full px-4 py-3 bg-white border-2 rounded-xl transition-all duration-300 focus:outline-none focus:border-black focus:ring-2 focus:ring-black/10 hover:border-gray-400 resize-none ${
                  errors.businessDescription ? 'border-red-500 focus:border-red-500 focus:ring-red-500/10' : 'border-gray-200'
                }`}
              />
        ...
        return (
          ...
          >
            <div className="text-center mb-8">
              ...
              <h3 className="text-2xl font-bold text-gray-800">Location & Hours</h3>
              <p className="text-gray-600 mt-2">Set your business location and operating hours</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <ToggleSwitch
                ...
                onChange={(value) => handleInputChange('deliveryAvailable', value)}
              ...
              <GlassCard className="p-4">
                <label className="flex items-center space-x-3 cursor-pointer">
                  ...
                  />
                  <div>
                    <span className="font-semibold text-gray-700">Home Service</span>
                    <p className="text-sm text-gray-500">I can provide services at customer location</p>
                  </div>
                </label>
              </GlassCard>
            </div>
        ...
        return (
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 mt-2">Upload required documents for verification</p>
            </div>
            ...
            {Object.values(errors).filter(Boolean).length > 0 && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                <p className="text-red-700 font-medium">Please upload all required documents:</p>
              ...
              <GlassCard className="p-6">
                ...
                <div className="grid md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium">{formData.contactNumber}</span>
                  </div>
              ...
              <GlassCard className="p-6">
                ...
                <div className="grid md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium">{formData.yearsInBusiness}</span>
                  </div>
                ...
                <div className="grid md:grid-cols-2 gap-4 text-sm">
                  {formData.location && (
                    <div className="md:col-span-2">
                      ...
                      <span className="ml-2 font-medium">{formData.location.address}</span>
                    </div>
                  )}
                  <div>
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium capitalize">{formData.weeklyOff || 'None'}</span>
                  </div>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="ml-2 flex flex-wrap gap-2 mt-1">
                        {formData.paymentMethods.map(method => (
                          <span key={method} className="bg-blue-100 text-blue-800 px-2 py-1 rounded-lg text-xs font-medium">
                            {method}
                          </span>
                        ))}
                      </div>
              ...
              <GlassCard className="p-6">
                ...
                <div className="flex gap-4">
                  <div className={`px-4 py-2 rounded-lg border-2 ${
                    formData.deliveryAvailable ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 text-gray-500'
                  }`}>
                    <span className="font-medium">Delivery: </span>
                    <span>{formData.deliveryAvailable ? 'Available' : 'Not Available'}</span>
                  </div>
                  <div className={`px-4 py-2 rounded-lg border-2 ${
                    formData.homeServiceAvailable ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-200 text-gray-500'
                  }`}>
                    <span className="font-medium">Home Service: </span>
                    <span>{formData.homeServiceAvailable ? 'Available' : 'Not Available'}</span>
                  </div>
                </div>
              </GlassCard>
                  ...
                  {[
                    ...
                    <div key={key} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                        formData[key] ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-500'
                      }`}>
                        {formData[key] ? <CheckCircle size={16} /> : <Upload size={16} />}
                      </div>
                      <div>
                ...
                <label className="flex items-start space-x-3 cursor-pointer">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      ...
                      I confirm that all the information provided is accurate and I have the authority to register this business.
                    </p>
                    {errors.acceptTerms && (
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
            ...
            <div className="flex items-center justify-between mt-12 pt-8 border-t border-gray-200">
              ...
              {currentStep < 5 ? (
                ...
                <motion.button
                  ...
                  className="flex items-center gap-2 px-8 py-3 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
        ...
        >
          ...
          <p className="text-sm text-gray-500 mt-6 max-w-2xl mx-auto">
            By registering, you agree that your information will be verified for authenticity. 
            We're committed to maintaining a trustworthy marketplace for all our users.
          </p>
        </motion.div>
      </div>
    </div>
